...
Current scope: follow [PLAN.md](PLAN.md) and [REGION_PLAN.md](REGION_PLAN.md)
for the v7 migration. Earlier wave topology is historical. Inventory generator
ownership before changing events: `build_story.py` can recreate old travel
actors and rooms. Preserve intentional v7 story events; the source-trigger
cleanup below applies to donor events during import, not to existing authored
...
For v7, story flag IDs are explicitly allocated in `story_flags.json` and
generated by `build_story.py`; never derive IDs from event order. Existing
item slots 0–45 are now uniquely assigned within `POX_HIDDEN_ITEMS_BASE`'s
reserved range. Run `python3 plastic_ox/alpha/audit_alpha.py` after adding flags.
Legacy wave importers must not allocate v7 flags. See PLAN.md for old-save
limitations caused by the corrected historical item aliases.
...
"gMapGroup_PlasticOxDungeons"     // verify existing registrations; v7 uses Seafoam, Meteor Falls and Hoenn Victory Road
```
Append names to `group_order` too. No cap issues until group 256.

## 7. Verification gate per wave

For v7, use the milestone acceptance gates in PLAN.md. Record donor revision,
source map/layout, destination group, port coordinates, tilesets, events and
encounter policy for each import. Run the deterministic HNS audit before and
after changes. In addition to a build, verify landing collision/elevation,
reciprocal warps, fresh screenshots, and post-transition stability. Test both
directions for ordinary links and blocked reverse travel for one-way ledges.
Water routes require Surf-aware tests. A debug placement or NPC town-skip warp

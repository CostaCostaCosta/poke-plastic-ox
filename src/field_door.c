...
// Plastic Ox: Pallet Town door animations are compiled in all builds (map stitching demo).
...
// Plastic Ox: Pallet Town door palettes are compiled in all builds (map stitching demo).
...
{
    const struct DoorAnimFrame *doorAnimFrames = LAYOUT_USES_FRLG_DOORS(gMapHeader.mapLayout->layoutVersion) ? sDoorAnimFrames_OpenSmallFrlg : sDoorOpenAnimFrames;
...
static s8 StartDoorOpenAnimation(const struct DoorGraphics *gfx, u32 x, u32 y)
{
    ...
    {
        if (gfx->size == 2)
            return StartDoorAnimationTask(gfx, LAYOUT_USES_FRLG_DOORS(gMapHeader.mapLayout->layoutVersion) ? sDoorAnimFrames_OpenLargeFrlg : sBigDoorOpenAnimFrames, x, y);
        else
            return StartDoorAnimationTask(gfx, LAYOUT_USES_FRLG_DOORS(gMapHeader.mapLayout->layoutVersion) ? sDoorAnimFrames_OpenSmallFrlg : sDoorOpenAnimFrames, x, y);
    }
}
...
static s8 StartDoorCloseAnimation(const struct DoorGraphics *gfx, u32 x, u32 y)
{
    ...
    if (gfx == NULL)
        return -1;
    else if (gfx->size != 1 && LAYOUT_USES_FRLG_DOORS(gMapHeader.mapLayout->layoutVersion))
        return StartDoorAnimationTask(gfx, sDoorAnimFrames_CloseLargeFrlg, x, y);
    else
        return StartDoorAnimationTask(gfx, LAYOUT_USES_FRLG_DOORS(gMapHeader.mapLayout->layoutVersion) ? sDoorAnimFrames_CloseSmallFrlg : sDoorCloseAnimFrames, x, y);
}

...
ifeq ($(METAMON_HEAP_TRACE),1)
  override CPPFLAGS += -DMETAMON_HEAP_TRACE=1
endif
ifeq ($(RELEASE),1)
...
.PHONY: all rom agbcc modern compare check debug release triggers battle
.PHONY: $(RULES_NO_SCAN)
...
# -DPLASTIC_OX_BUILD_BATTLE=1 (Oak team gift + Lance battle visible).
battle: all
